...
public class MatchScoreCalculationService {
  ...
  private boolean isTieBreakCompleted(Match match) {
    if (match.getPlayer1().getPlayerScore().getTieBreakPoint() >= 7
        || match.getPlayer2().getPlayerScore().getTieBreakPoint() >= 7) {
  ...
  private int getPointsDifference(Player scoringPlayer, Player losingPlayer) {
    return Math.abs(
        scoringPlayer.getPlayerScore().getPoint().getNumericValue()
            - losingPlayer.getPlayerScore().getPoint().getNumericValue());
  }

  private int getTieBreakPointDifference(Match match) {
    return Math.abs(
        match.getPlayer1().getPlayerScore().getTieBreakPoint()
            - match.getPlayer2().getPlayerScore().getTieBreakPoint());
  }

  private int getGameDifference(Match match) {
    return Math.abs(
        match.getPlayer1().getPlayerScore().getGame()
            - match.getPlayer2().getPlayerScore().getGame());
  }
